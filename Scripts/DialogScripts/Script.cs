...
namespace DialogScripts
{
    ...
    {
        public ScriptType ScriptType { get; set; } = ScriptType.NpcDialog;
        ...
        {
            MethodInfo script;
            if (Scripts.TryGetValue(npcId, out script))
                return (bool)script.Invoke(null, new object[] { player, npcId, control, input});
            if (Scripts.TryGetValue(player.CurrentNpcId, out script))
                return (bool)script.Invoke(null, new object[] { player, player.CurrentNpcId, control, input });
            return false;
        }
