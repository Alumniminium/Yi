            ...
            try
            {
                ...
                switch (command[0])
                {
                    ...
                    case "spawn":
                        {
                            ...
                            {
                                case "0":
                                    Helpers.Message.SendTo(player, "Removing mobs...", MsgTextType.Center);
                                    foreach (var map in GameWorld.Maps.Values)
                                    {
                                        foreach (var mapObject in map.Entities.Values.OfType<Monster>())
                                        {
                                            map.Leave(mapObject);
                                        }
                                    }
                                    Collections.Monsters.Clear();
                                    Helpers.Message.SendTo(player, "Removed all mobs!", MsgTextType.Center);
                                    break;
                                case "+":
                                    Helpers.Message.SendTo(player, "Increasing spawns...", MsgTextType.Center);

                                    foreach (var kvp in Collections.Spawns)
                                    {
                                        var id = kvp.Key;
                                        var spawn = kvp.Value;

                                        if (spawn.MobId > 899 && spawn.MobId < 911)
                                            continue;
                                        spawn.Amount += 3;
                                        counter += spawn.Amount;
                                        Collections.Spawns[id] = spawn;
                                    }
                                    Helpers.Message.SendTo(player, $"Increased spawns to {counter} mobs", MsgTextType.Center);
                                    break;
                                case "-":
                                    Helpers.Message.SendTo(player, "Increasing spawns...", MsgTextType.Center);
                                    counter = 0;
                                    ...
                                    Helpers.Message.SendTo(player, $"Decreased spawns to {counter} mobs", MsgTextType.Center);

                                    break;
                                case "=":
                                    await Task.Run(() =>
                                    {
                                        Helpers.Message.SendTo(player, "Spawning mobs...", MsgTextType.Center);
                                        counter = Collections.Spawns.Select(kvp => kvp.Value).Aggregate(counter, (current, spawn) => current + spawn.Amount);
                                        MonsterDb.Spawn(counter).Wait();
                                        foreach (var mapObject in GameWorld.Maps[player.MapId].Entities.Values.OfType<Monster>())
                                        {
                                            ScreenSystem.Update(mapObject);
                                            ScreenSystem.SendSpawn(mapObject);
                                        }
                                        Helpers.Message.SendTo(player, $"Spawned {Collections.Monsters.Count} monsters!", MsgTextType.Center);
                                    });
                                    break;
                            }
                            break;
                    ...
                    case "test":
                        ...
                        break;
                    case "setspawn":
                        if (GameWorld.Maps.TryGetValue(player.MapId, out var fmap))
                        {
                            fmap.SpawnVector2 = new Vector2(player.Location.X, player.Location.Y);
                            player.GetMessage("SYSTEM", player.Name, $"Spawn Point set to {player.Location.X}-{player.Location.Y} SAVE THE DB NOW!", MsgTextType.Center);
                        }
                        else
                        {
                            player.GetMessage("SYSTEM", player.Name, $"SPAWN POINT NOT SET! MAP DIDNT EXIST ON THE SERVER!", MsgTextType.Center);
                        }
                        break;
                    case "suck":
